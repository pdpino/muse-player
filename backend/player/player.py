...
class MusePlayer:
    ...
    def initialize_engine(self, stream_type, regulator_type, eeg_mode=None, eeg_n=None, waves_selected=None, accum_samples=None):
        ...
        if stream_type == "eeg":
            ...
            self.generator = engine.EEGRawYielder(eeg_mode, stream_n=eeg_n)
            ...
            self.generator = engine.WaveProcessor(wave_yielder)
            ...
            self.generator = engine.WaveProcessor(self.feel_processor)
        ...
        self.eeg_engine = engine.EEGEngine(stream_type, self.eeg_collector, self.eeg_buffer, self.generator)
    ...
    def initialize_flask(self, ip, url, port):
        ...
        # Stream thread
        self.stream = threading.Thread(target=app.run, kwargs={"host": ip, "port": port})
...
#     self.commands.add_command("-1", self.generator.signal_start_calibrating, info.start_calib_mark, "notification")
#     self.commands.add_command("-2", self.generator.signal_stop_calibrating, info.stop_calib_mark, "notification")
